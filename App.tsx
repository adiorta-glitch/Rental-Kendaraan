...
// Perubahan: Hapus import initializeData() yang bersifat sinkron
import { getStoredData, DEFAULT_SETTINGS, initializeData } from './services/dataService'; 
...
// Perubahan: Hapus initializeData() di luar komponen. Panggilan data sekarang ada di useEffect.
// initializeData(); 
// --- THEME ENGINE DAN SIDEBAR ITEMS TIDAK BERUBAH ---
...
const ThemeEngine = ({ settings }: { settings: AppSettings }) => {
    // KODE THEME ENGINE LAMA (TIDAK BERUBAH)
    useEffect(() => {
        // 1. Handle Dark Mode
...
// ... (SidebarItem dan BottomNavItem TIDAK BERUBAH) ...
...
  const [settings, setSettings] = useState<AppSettings | null>(null); // Perubahan: Inisialisasi dengan null
  const [loadingSettings, setLoadingSettings] = useState(true); // Perubahan: Tambahkan loading state
  
  // Role Helpers
...
  useEffect(() => {
    async function loadSettings() {
        try {
            // Perubahan: Panggil initializeData ASYNC (untuk setting default di Firestore)
            await initializeData(); 
            
            // Perubahan: Ambil settings dari Firebase (fungsi getStoredData sudah dimodifikasi)
            const loadedSettings = await getStoredData<AppSettings>('appSettings', DEFAULT_SETTINGS); 
            
            setSettings(loadedSettings);
        } catch (error) {
            console.error("Gagal memuat pengaturan dari Firestore, menggunakan default:", error);
            setSettings(DEFAULT_SETTINGS); // Fallback ke default jika Firebase gagal
        } finally {
...
  }, []);
    ...
    if (loadingSettings || !settings) {
        // Tampilkan loading screen sementara settings dimuat dari database
        return (
            <div className="flex justify-center items-center min-h-screen bg-slate-50 dark:bg-slate-900">
                <p className="text-xl font-medium text-slate-700 dark:text-slate-300">Memuat Pengaturan...</p>
            </div>
        );
    }
...
      <ThemeEngine settings={settings!} /> {/* Paksa settings karena loading sudah dicek */}

      {/* Sidebar (Desktop) */}
      {/* KODE SIDEBAR LAMA (TIDAK BERUBAH) */}
...
        <nav className="p-4 space-y-1 overflow-y-auto max-h-[calc(100vh-140px)] custom-scrollbar">
          {/* KODE NAVIGASI LAMA (TIDAK BERUBAH) */}
          {!isDriver && !isPartner && (
            <>
...
      {/* Mobile Sidebar (Slide-over) dan Main Content TIDAK BERUBAH */}
      {/* ... (lanjutan kode AppLayout lama) ... */}
